...
# ---------------------------------------------------------
# Code By GIS Innovation - Ver 4.0.0
# Changelog:
...
from PyQt6.QtWidgets import (
    ...
    QTreeView, QListWidget, QListWidgetItem, QSplitter, QAbstractItemView
)
from PyQt6.QtCore import Qt, QCoreApplication, QDir, QFileInfo, QSize
from PyQt6.QtGui import QStandardItemModel, QStandardItem, QIcon, QPainter, QColor
from qgis.core import (
    QgsProcessingAlgorithm, QgsProcessingParameterFeatureSource,
    QgsProcessingParameterString, QgsProcessingParameterFeatureSink,
    QgsProcessingOutputString, QgsVectorLayer, QgsVectorFileWriter, Qgis, QgsMessageLog, QgsProcessing, QgsProcessingParameterDefinition
)
...
def _fme_exe_name():
    """Return the FME executable filename for the current platform."""
    return 'fme.exe' if sys.platform == 'win32' else 'fme'


def _fmeworkbench_exe_name():
    """Return the FME Workbench executable filename for the current platform."""
    return 'fmeworkbench.exe' if sys.platform == 'win32' else 'fmeworkbench'


def _fme_default_install_dir():
    """Return the most likely FME installation directory for the current platform."""
    if sys.platform == 'win32':
        return r'C:\Program Files\FME'
    elif sys.platform == 'darwin':
        return '/Applications/FME'
    else:
        return '/opt/fme'


def _fme_default_exe():
    """Return a platform-appropriate default path for the FME executable."""
    return os.path.join(_fme_default_install_dir(), _fme_exe_name())


def _filesystem_roots():
    """Return a list of filesystem root paths for the current platform."""
    if sys.platform == 'win32':
        roots = []
...
class CollapsibleGroupBox(QGroupBox):
    def __init__(self, title):
        ...
        self.toggle_button.setArrowType(Qt.ArrowType.RightArrow)  # Start with the right arrow (collapsed)
    ...
    def toggle(self):
        ...
        if self.toggle_button.isChecked():
            self.toggle_button.setArrowType(Qt.ArrowType.DownArrow)  # Change to down arrow (expanded)
            self.content.setVisible(True)
        else:
            self.toggle_button.setArrowType(Qt.ArrowType.RightArrow)  # Change back to right arrow (collapsed)
            self.content.setVisible(False)
...
class QGISFMEFormAlgorithmAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        command_param = QgsProcessingParameterString(
            ...
            defaultValue=f'"{_fme_default_exe()}" "path/to/workspace.fmw"' # Default value to make it non-mandatory
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            ...
            if input_layer is not None:
                ...
                # If no input layer, ensure the input_geojson path is still set for the command
                # Potentially create an empty placeholder or handle this case differently if FME requires it
                # For now, we'll pass the path, FME might handle missing files gracefully or error out
                pass # input_geojson path is already defined

            # Prepare the command list using shlex
            try:
                cmd_list = shlex.split(command) # Split base command safely
            except ValueError as e:
                feedback.reportError(f"Error parsing the base FME command: {e}. Check quoting.")
                raise QgsProcessingException(f"Error parsing FME command: {e}")

            # Append GeoJSON parameters to the list
            if input_layer is not None:
                cmd_list.extend(['--SourceDataset_GEOJSON', input_geojson])
            cmd_list.extend(['--DestDataset_GEOJSON', output_geojson])
            
            feedback.pushInfo(f"Running FME command: {' '.join(cmd_list)}") # Log the reconstructed command for info
            # Run the command as a list, without shell=True
            _kwargs = {'capture_output': True, 'text': True, 'check': False}
            if sys.platform == 'win32':
                _kwargs['creationflags'] = subprocess.CREATE_NO_WINDOW
            result = subprocess.run(cmd_list, **_kwargs) # check=False to handle errors manually
            
            feedback.pushInfo(result.stdout)
            if result.returncode != 0:
                # Report error if FME process exited with non-zero code
                error_message = f"FME process failed with exit code {result.returncode}. Error output:\n{result.stderr}"
                feedback.reportError(error_message)
                QgsMessageLog.logMessage(error_message, "FME Connector", level=Qgis.Critical)
                # Optionally raise exception to stop processing
                # raise QgsProcessingException(error_message)
            elif result.stderr: # Log stderr even if return code is 0, as FME might put warnings there
                feedback.pushWarning(f"FME process generated warnings/errors on stderr:\n{result.stderr}")
                QgsMessageLog.logMessage(f"FME stderr output:\n{result.stderr}", "FME Connector", level=Qgis.Warning)
...
class FMEFileLister(QWidget):
    ...
    def open_workspace_in_fme(self):
        """Open the selected workspace in FME Workbench (platform-aware)."""
        import subprocess
        ...
        if not os.path.exists(workspace_path):
            ...
            return
        # Try to find the FME Workbench executable in the same folder as fme executable
        wb_name = _fmeworkbench_exe_name()
        workbench_exe = None
        fme_exe = self.fme_exe_path
        if fme_exe and os.path.exists(fme_exe):
            workbench_exe = os.path.join(os.path.dirname(fme_exe), wb_name)
            if not os.path.exists(workbench_exe):
                workbench_exe = None
        if not workbench_exe:
            start_dir = os.path.dirname(fme_exe) if fme_exe else _fme_default_install_dir()
            if sys.platform == 'win32':
                file_filter = f"FME Workbench Executable ({wb_name})"
            else:
                file_filter = f"FME Workbench Executable ({wb_name});;All Files (*)"
            exe_path, _ = QFileDialog.getOpenFileName(self, "Select FME Workbench Executable", start_dir, file_filter)
            if exe_path and os.path.basename(exe_path).lower() == wb_name.lower():
                workbench_exe = exe_path
                # Save for session
                self.fmeworkbench_exe_path = exe_path
            elif exe_path:
                QMessageBox.warning(self, "Invalid Selection", f"Please select the correct {wb_name} file.")
                return
            else:
                return
        ...
        try:
            ...
            subprocess.Popen([workbench_exe, workspace_path], **_popen_kwargs)
        except Exception as e:
    ...
    def select_fme_exe_from_table(self):
        exe_name = _fme_exe_name()
        if sys.platform == 'win32':
            file_filter = f"FME Executable ({exe_name})"
        else:
            file_filter = f"FME Executable ({exe_name});;All Files (*)"
        exe_path, _ = QFileDialog.getOpenFileName(self, "Select FME Executable", _fme_default_install_dir(), file_filter)
        if exe_path and os.path.basename(exe_path).lower() == exe_name.lower():
            ...
            QMessageBox.warning(self, "Invalid Selection", f"Please select the correct {exe_name} file.")
    ...
    def build_ui(self):
        ...
        # Add all filesystem roots (drives on Windows, / on Linux/macOS)
        for root in _filesystem_roots():
            drive_item = QStandardItem(self.drive_icon, root)
            drive_item.setData(root, Qt.ItemDataRole.UserRole)
        ...
        selector_splitter = QSplitter(Qt.Orientation.Horizontal)
        ...
        self.user_parameters_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
        self.user_parameters_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        ...
        self.paths_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
        ...
        self.paths_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        about_text = QLabel(
            "<b>FME Workspace Connector</b><br><br>"
            "This plugin allows you to:<br>"
            "&bull; Browse and select FME workspaces<br>"
            "&bull; View workspace parameters and datasets<br>"
            "&bull; Configure and run FME transformations<br><br>"
            "<b>Version:</b> 4.0.0<br>"
            "<b>Company:</b> GIS Innovation Sdn. Bhd.<br>"
            "<b>Website:</b> <a href='https://www.gis.com.my'>www.gis.com.my</a><br><br>"
            "If you would like to contact us, send us an email at "
            "<a href='mailto:sales@gis.fm'>sales@gis.fm</a>"
        )
    ...
    def populate_tree(self, parent_item, path):
        ...
        dir = QDir(path)
        dir.setFilter(QDir.Filter.AllDirs | QDir.Filter.NoDotAndDotDot | QDir.Filter.Hidden)
    ...
    def on_fmw_selected(self, item):
        ...
        fmw_path = item.data(Qt.ItemDataRole.UserRole)
        if not fmw_path:
            return
    ...
    def build_fme_command(self):
        ...
        try:
            ...
            if not fme_path or fme_path == "Not set" or not os.path.exists(fme_path):
                fme_path = os.getenv('FME_HOME', _fme_default_exe())
